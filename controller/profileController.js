...
export const buildProfileResponse = async (
  ...
  if (role === 'ecommerce_seller') {
    ...
    totalFollowers =
      stores?.reduce((sum, store) => sum + (store.followers || 0), 0) || 0
    totalViews =
      stores?.reduce((sum, store) => sum + (store.views || 0), 0) || 0
  ...
  if (role && user.profiles && user.profiles.get(role)) {
    ...
    if (roleProfile && typeof roleProfile === 'object') {
      if (roleProfile._doc) {
        ...
        typeof roleProfile.toObject === 'function'
      ) {
  ...
  const firstName =
    profileData.firstName || (user.name ? user.name.split(' ')[0] : '') || ''
  const lastName =
  ...
  try {
    ...
    if (existingProfile) {
      ...
      if (existingProfile._doc) {
        ...
        typeof existingProfile.toObject === 'function'
      ) {
    ...
    if (specialization !== undefined)
      updatedProfile.specialization = specialization
    ...
    if (firstName !== undefined || lastName !== undefined) {
      const newFirstName =
        firstName !== undefined ? firstName : existingProfilePlain.firstName
      const newLastName =
        lastName !== undefined ? lastName : existingProfilePlain.lastName
          ...
          name: {
            $let: {
              vars: {
                ...
                last: { $ifNull: ['$profiles.realestate_seller.lastName', ''] }
              },
              in: {
                $cond: {
                  if: {
                    $and: [{ $ne: ['$$first', ''] }, { $ne: ['$$last', ''] }]
                  },
